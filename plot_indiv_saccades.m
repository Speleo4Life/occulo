...
csv_fname = uigetfile("*.csv", "Choose cooresponding csv");
...
for ind = 1:length(event_struct.time_series)
    ...
    if extract
        ...
        cutoffs = findchangepts(eog_saccade, "MaxNumChanges", 2); % Isolate saccade using changepoints
        ...
        figure()
        hold on;
        plot(eog_time, eog_saccade, 'DisplayName','Raw EOG');
        plot(elink_time, elink_saccade, 'DisplayName','Elink');
        for i = 1:numel(filters)
            filtered_data = filtered_eog.(filters{i});
            to_plot = filtered_data(eog_inds);
            plot(eog_time, to_plot, 'DisplayName', filters{i});
        end
