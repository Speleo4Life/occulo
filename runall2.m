...
PX_TO_DEGREE_EL = 0.06537277453582728;
IN_PATH = 'C:\Users\Vision Lab\Development\Help_with_Data_Processing\XDFS\';

%% Get Data for Import
pid = {'P026','P020','P022','P023','P024','P025'};
%'P019','P020','P022','P023','P024'
%'P025','P026','P027','P028','P029'
%'P030','P032','P033','P034','P035'
   
for pd = 1:length(pid)
    % [xdf_fname, xdf_path] = uigetfile('*.xdf', 'Select a File','MultiSelect','on');
    xdf_path = IN_PATH;
    xdf_files = dir([IN_PATH pid{pd} '*.xdf']);
    y = struct2cell(xdf_files);
    xdf_fname = y(1,:);
    ...
    curr_pid_in_table = strcmp(pid{pd}, {'P019','P020','P021','P022','P023','P024','P025'});
    if any(curr_pid_in_table)
       calib_rand_table = readtable('rand_calib_trials.csv');
       calib_row = calib_rand_table(curr_pid_in_table,:);
       calib_rand_trials = [calib_row.Rand1, calib_row.Rand2, calib_row.Rand3];
    else
        calib_rand_trials = [];
    end
    ...
    for II = 1:numel(xdf_fname)
        ...
        ob_dat_x = fillmissing(ob_dat_x, "pchip");
        el_dat_x = fillmissing(el_dat_x, "pchip");
        ...
        if contains(xdf_fname{II}, 'C1')
            calibration_factor_eog = linear_calibration(event_struct,...
                opbci_struct, opbci_threshold, false, calib_rand_trials);
%             calibration_factor_elink = linear_calibration(event_struct,...
%                 elink_struct, elink_threshold, false, calib_rand_trials);
            calibration_factor_elink = PX_TO_DEGREE_EL;
        end

        ob_features = get_features(event_struct, opbci_struct,...
            calibration_factor_eog, "EOG", remove_outliers, xdf_fname{II});

        if ~contains(xdf_fname{II}, 'C4')
            elink_features = get_features(event_struct, elink_struct,...
                calibration_factor_elink, "EyeLink", remove_outliers, xdf_fname{II});
        else
            elink_features = ob_features;
%         	elink_features = struct();
        end
    %     
